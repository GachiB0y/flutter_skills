...
Ключевой момент: middleware принимает **callback'и** (`getToken`, `logout`), а не контроллер напрямую. Это позволяет:
...
3. Auth middleware принимает callback'и, не контроллер
