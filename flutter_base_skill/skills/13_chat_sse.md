...
class AssistantMessageEvent extends ChatEvent {
  final String id;
  final String text;
  final DateTime timestamp;
...
class PingEvent extends ChatEvent {
  const PingEvent();
}
```
...
SSE-подписка создаётся в **конструкторе контроллера**:
...
> "В dispose я отписываюсь от таймера проверки жизнеспособности и от подписки на репозиторий."

## Состояние подписки -- в контроллере, не в репозитории

```dart
// lastUpdate -- это мутабельная переменная = состояние
// Состояние должно быть в контроллере, не в репозитории
DateTime _lastUpdate = DateTime.now();
StreamSubscription? _subscription;
Timer? _heartbeatTimer;
```

> "Подписка, мутабельная переменная -- это состояние. Состояние должно быть в контроллере. У репозиториев состояния быть не должно."

### Почему не в репозитории

- Может быть несколько контроллеров одновременно (несколько чатов открыто)
- Они будут драться за общий `lastUpdate`
- При dispose контроллера нужно отписаться **только от своей** подписки
...
class ChatRepositoryImpl implements ChatRepository {
  @override
  Stream<ChatEvent> subscribe(String chatId) async* {
...
3. Состояние подписки (lastUpdate, subscription, timer) -- **в контроллере**, не в репозитории
4. Сообщения пользователя приходят через SSE, а не как ответ на POST
5. Dispose: отменить таймер + отменить подписку
6. У репозиториев **нет** dispose и **нет** состояния (кроме эфемерного кэша)
7. Несколько контроллеров чата могут существовать одновременно -- каждый со своей подпиской
