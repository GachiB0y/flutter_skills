...
Через ControllerObserver все переходы стейтов логируются:

```
AuthController: Idle(initial) -> Processing(logging in with Google)
AuthController: Processing -> Idle(authenticated)
ChatListController: Idle(initial) -> Processing(refreshing chats)
ChatListController: Processing -> Idle(5 chats received)
```
